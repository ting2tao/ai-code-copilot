...
def project_context(detected):
    lines = [
        ...
        "> Existing project edits are preserved; changed generated files are written as `.new` candidates.",
        "",
        "## 1. 应用概况",
        "",
...
def state_content(detected):
    ...
    data["syncPolicy"] = "missing files are created; changed rules/templates are written as .new candidates; this state file is managed automatically"
...
for src in sorted((copilot_home / "changes" / "templates").glob("*.md")):
    status, path = copy_if_missing_or_new(src, changes_target / "templates" / src.name)
    events.append((status, path))

status, path = write_if_missing_or_new(rules_target / "project-context.md", project_context(detected))
events.append((status, path))
