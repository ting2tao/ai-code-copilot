...
usage() {
  cat <<'EOF'
Usage: scripts/init_project.sh [--project <dir>] [--sync]
...
python3 - "$COPILOT_HOME" "$PROJECT_DIR" "$MODE" <<'PY'
import json
import os
import shutil
import sys
...
def project_context(detected):
    lines = [
        ...
        "# 工程上下文",
        "",
        f"> Generated by ai-code-copilot `{mode}` on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}.",
...
print(f"ai-code-copilot {mode} complete")
